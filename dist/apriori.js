...
var Apriori = /** @class */ (function(_super) {
  ...
  Apriori.prototype.exec = function(transactions, cb) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      var frequentItemsets = [
        _this.getFrequentOneItemsets(_this._transactions)
      ];
      var i = 0;
  ...
  Apriori.prototype.getFrequentOneItemsets = function(transactions) {
    ...
    return Object.keys(count).reduce(function(ret, stringifiedItem) {
  ...
  Apriori.prototype.getFrequentKItemsets = function(frequentNItemsets) {
    ...
    var items = frequentNItemsets
      .reduce(function(items, itemset) {
        return items.concat(itemset.items);
      }, [])
      .filter(function(item, index, that) {
        return that.indexOf(item) === index;
      });
    // Generating candidates and counting their occurence.
    return this._getCandidatesCount(this._generateKCandidates(items, k)).filter(
      function(itemset) {
        var isFrequent =
          itemset.support >= _this._transactions.length * _this._support;
        if (isFrequent) _this.emit("data", itemset);
        return isFrequent;
      }
    );
  };
  ...
  Apriori.prototype._generateKCandidates = function(items, k) {
    ...
    var _loop_1 = function(i) {
      ...
      this_1
        ._generateKCandidates(items.slice(i + 1), k - 1)
        .forEach(function(tailcomb) {
          return ret.push({
  ...
  Apriori.prototype._getCandidatesCount = function(candidates) {
    this._transactions.forEach(function(transaction) {
      candidates.forEach(function(candidate) {
        var includesCandidate = candidate.items.every(function(item) {
          return transaction.indexOf(item) !== -1;
        });
